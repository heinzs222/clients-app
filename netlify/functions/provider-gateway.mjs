...
function googleConversion(config, input) {
  const email = normalizeEmail(input.email);
  const phone = normalizePhone(input.phone, input.country);
  const firstName = cleanString(input.first_name, 200).toLowerCase();
  const lastName = cleanString(input.last_name, 200).toLowerCase();
  const streetAddress = cleanString(input.address1, 500).toLowerCase();
  const city = cleanString(input.city, 200);
  const state = cleanString(input.state, 100);
  const postalCode = cleanString(input.postal_code, 40);
  const countryCode = cleanString(input.country, 2).toUpperCase() || config.country;
  const identifiers = [];
  ...
  if (firstName && lastName && countryCode) {
    identifiers.push({
      addressInfo: removeEmpty({
        hashedFirstName: sha256(firstName),
        hashedLastName: sha256(lastName),
        hashedStreetAddress: streetAddress ? sha256(streetAddress) : undefined,
        city,
        state,
        postalCode,
        countryCode
...
function rawTrackerSource(config) {
  const publicConfig = {
    route: config.route,
...
function trackerSource(config) {
  return rawTrackerSource(config)
    .replace(
      'country:field(f,["country"])||"US",client_user_agent',
      'address1:field(f,["address1","address_line_1","street_address","street"]),city:field(f,["city","locality"]),state:field(f,["state","region","province"]),postal_code:field(f,["postal_code","postcode","zip"]),country:field(f,["country"])||C.country||"US",client_user_agent'
    )
    .replace(
      'address:{first_name:p.first_name,last_name:p.last_name,country:p.country}',
      'address:{first_name:p.first_name,last_name:p.last_name,street:p.address1,city:p.city,region:p.state,postal_code:p.postal_code,country:p.country}'
    );
}
