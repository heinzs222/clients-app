...
function splitName(value) {
  const parts = cleanString(value, 200).toLowerCase().split(/\s+/).filter(Boolean);
...
function tiktokPayload(config, input, request, pageUrl) {
  ...
  const names = splitName(input.full_name || `${cleanString(input.first_name)} ${cleanString(input.last_name)}`);
    ...
    body: removeEmpty({
      ...
      data: [{
        ...
        user: {
          ...
          first_name: names.first ? sha256(names.first) : undefined,
          last_name: names.last ? sha256(names.last) : undefined
        },
        page: {
          url: pageUrl,
