...
import { AlertCircle, Check, CheckCircle2, Clipboard, LoaderCircle, X } from "lucide-react";
...
function RouteLink({ route, navigate, className = "", children, ariaLabel }) {
  ...
  return (
    <a className={className} href={publicHrefs[route] || "/"} aria-label={ariaLabel}
...
export function Brand({ compact = false }) {
  return (
    <span className={`brand brandImageOnly ${compact ? "compact" : ""}`}>
      <img className="brandLogo" src="/capi-tracker-logo.png" alt="Simple CAPI" width="1241" height="239" />
    </span>
  );
}
  ...
  return (
    <header className="publicHeader">
      <div className="publicHeaderInner">
        <RouteLink className="brandButton" route="home" navigate={navigate} ariaLabel="Simple CAPI home"><Brand compact /></RouteLink>
        ...
        <div className="publicActions">
          {user ? (
            navigate ? <button className="button primary small" type="button" onClick={() => navigate("dashboard")}>Open dashboard</button>
  ...
  return (
    <footer className="publicFooter">
      <div className="publicFooterInner">
        <div><Brand compact /><p>&copy; 2026 Simple CAPI. All rights reserved.</p></div>
...
export function Field({ label, hint, error, children, className = "" }) {
  return <label className={`field ${className}`}><span className="fieldLabel">{label}</span>{children}{error ? <small className="fieldError">{error}</small> : hint ? <small>{hint}</small> : null}</label>;
}

export function InputShell({ icon: Icon, action, children, code = false }) {
  return <span className={`inputShell ${code ? "code" : ""}`}>{Icon ? <Icon size={18} aria-hidden="true" /> : null}{children}{action}</span>;
}

export function Notice({ tone = "info", title, children }) {
  const Icon = tone === "success" ? CheckCircle2 : AlertCircle;
  return <div className={`notice ${tone}`} role={tone === "error" ? "alert" : "status"}><Icon size={19} /><div>{title ? <strong>{title}</strong> : null}<span>{children}</span></div></div>;
}

export function Spinner({ label = "Loading" }) {
  return <span className="spinnerLabel" role="status"><LoaderCircle className="spin" size={18} /> {label}</span>;
}

export function CopyButton({ value, label = "Copy", compact = false, className = "" }) {
  const [copied, setCopied] = useState(false);
  async function copy() {
    try { await copyText(value); setCopied(true); window.setTimeout(() => setCopied(false), 1400); }
    catch { setCopied(false); }
  }
  return <button className={`copyButton ${compact ? "compact" : ""} ${className}`} type="button" onClick={copy} title={copied ? "Copied" : label} aria-label={copied ? "Copied" : label}>{copied ? <Check size={16} /> : <Clipboard size={16} />}{!compact ? (copied ? "Copied" : label) : null}</button>;
}
...
export function EmptyState({ icon: Icon, title, children, action }) {
  return <div className="emptyState">{Icon ? <span><Icon size={24} /></span> : null}<h3>{title}</h3><p>{children}</p>{action}</div>;
}
  ...
  return (
    <div className="modalBackdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title">
        <header><h2 id="modal-title">{title}</h2><button className="iconOnly" type="button" onClick={onClose} aria-label="Close dialog" title="Close"><X size={19} /></button></header>
        <div className="modalBody">{children}</div>{actions ? <footer>{actions}</footer> : null}
      </section>
    </div>
  );
}
