      ...
      <main>
        <section className="homeHero">
          <div className="homeHeroCopy">
            ...
            <p>Simple CAPI launches an isolated endpoint for each client, keeps Meta credentials server-side, and pairs browser and server Lead and Schedule events with the same event ID.</p>
          ...
          <div className="pipelineVisual" aria-label="Form or GHL to Simple CAPI to Meta data flow">
            ...
            <div className="pipelineLog">
              ...
              <p className="active"><span>&gt;</span> Provisioning isolated client endpoint</p>
        ...
        <section className="featureBand">
          ...
          <div className="featureGrid">
            <article>
              ...
              <p>Each client and Meta dataset receives a separate event service and encrypted credential set.</p>
            </article>
            <article>
            ...
            <article>
              ...
              <p>Meta access tokens go only to the provisioner and remain in encrypted server-side storage.</p>
            </article>
          </div>
        </section>
...
const docsSections = [
  {
    ...
    body: (
      <>
        ...
        <div className="inlineFlow"><span>Browser form</span><ArrowRight size={16} /><span>GHL inbound webhook</span><ArrowRight size={16} /><span>Simple CAPI endpoint</span><ArrowRight size={16} /><span>Meta CAPI</span></div>
        <p>If no GHL inbound URL is configured, the tracker posts directly to the generated Simple CAPI endpoint.</p>
      </>
    )
  },
  {
  ...
  },
  {
    ...
    body: (
      <ol>
        ...
        <li>Use the generated Simple CAPI endpoint as the URL and paste the generated JSON mapping as the request body.</li>
  ...
  },
  {
    id: "deduplication",
    title: "Deduplication",
    body: <p>Meta deduplicates matching browser and server events using the same event name and event ID. The tracker fires the browser event and passes that exact ID through GHL to the server endpoint. Do not replace <code>event_id</code> with a contact ID in the workflow.</p>
  },
  {
    id: "multiple-pages",
    ...
    <PageFrame route="privacy" navigate={navigate} user={user}>
      <main className="contentPage legalPage">
        ...
        <article className="document wide">
          <section><h2>1. Scope</h2><p>This policy applies to the Simple CAPI web application and the client-specific event services it provisions. It does not replace the privacy policies of Meta, GoHighLevel, our managed cloud providers, or a website using the generated tracker.</p></section>
          ...
          <section><h2>3. Meta credentials</h2><p>A Meta Dataset ID and access token are submitted to the server-side provisioner over HTTPS. The token remains in encrypted server-side storage. It is not embedded in tracker code, saved to browser storage, logged by the application, or returned in API responses.</p></section>
          <section><h2>4. Endpoint configuration</h2><p>Non-secret endpoint details such as client name, Dataset ID, and deployment state are used to display and manage endpoints. Tracker preferences, including an optional GHL inbound webhook URL, are stored locally in the current browser for convenience.</p></section>
          <section><h2>5. Lead event data</h2><p>Generated endpoints process event data only to format and send it to Meta Conversions API. Simple CAPI does not provide a lead database and does not intentionally persist event payloads. Our infrastructure providers, GHL, and Meta may process data under their own terms and retention policies.</p></section>
  ...
  return (
    <PageFrame route="terms" navigate={navigate} user={user}>
      <main className="contentPage legalPage">
        ...
        <article className="document wide">
          <section><h2>1. Service</h2><p>Simple CAPI provisions client-specific event services, hosted tracker files, and installation guidance. Availability depends on our managed infrastructure, Meta, GHL, and internet services outside Simple CAPI's control.</p></section>
          ...
          <section><h2>7. Third-party platforms</h2><p>Your use of Lemon Squeezy, Meta, GHL, and managed infrastructure providers remains subject to their terms. Changes, outages, limits, token revocation, payment disputes, or enforcement decisions by those platforms may affect the service.</p></section>
      ...
      <main className="statusPage">
        ...
        <section className="statusPanel">
          <div>
            ...
            <div><h2>Endpoint provisioner</h2><p>{backend.message}</p></div>
          </div>
        ...
        {!ready && backend.status !== "checking" ? (
          <Notice tone="warning" title="Administrator action required">The provisioning service needs administrator credentials before endpoint creation can work.</Notice>
        ) : null}
